...
export function MoodChart({ entries }: MoodChartProps) {
  ...
  const getRangeLabel = () => {
    ...
    if (isAllTimeOption) {
      ...
      return `All Time (${format(actualStartDate, 'MMM d, yyyy')} - ${format(actualEndDate, 'MMM d, yyyy')})`;
    }

    const { startDate, endDate } = dateRange;
    const daysDiff = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));

    // Try to find if this matches a preset precisely
    const now = new Date();
    const isLast7 = daysDiff === 7 && format(endDate, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
    const isLast30 = daysDiff === 30 && format(endDate, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
    const isLast90 = daysDiff === 90 && format(endDate, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');

    if (isLast7) return 'Last 7 Days';
    if (isLast30) return 'Last 30 Days';
    if (isLast90) return 'Last 90 Days';

    // If it's a custom range or any other manually selected period
